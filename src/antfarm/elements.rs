...
use super::{get_surface_level, settings::Settings, Element, WORLD_HEIGHT, WORLD_WIDTH};
...
pub fn setup_elements(parent: &mut ChildBuilder, settings: &Res<Settings>) {
    ...
    let air_bundles =
        (1..get_surface_level(settings.initial_dirt_percent) + 1).flat_map(|row_index| {
            (0..WORLD_WIDTH).map(move |column_index| {
                // NOTE: row_index goes negative because 0,0 is top-left corner
    ...
    let dirt_bundles = ((get_surface_level(settings.initial_dirt_percent) + 1)..WORLD_HEIGHT)
        .flat_map(|row_index| {
            (0..WORLD_WIDTH).map(move |column_index| {
                // NOTE: row_index goes negative because 0,0 is top-left corner
